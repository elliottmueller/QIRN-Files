...
def fluxinvert(intermediates,networkbuilder,reactiondatabase, time, dt):
    ...
    reservoirs = []
    EClist = []
    conc_glucose=np.zeros(64)
    conc_pyruvate = np.zeros(8)
    conc_proline = np.zeros(32)
    conc_hco3=np.zeros(2)
    conc_co2_pyrucarb=np.zeros(2)
    conc_glyoxylate = np.zeros(4)
    conc_3PGA = np.zeros(8)
